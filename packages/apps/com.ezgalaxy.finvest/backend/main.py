...
# SDK shim — handles 404 gracefully (returns null instead of throwing for GET on missing records)
SDK_JS = """(function(){'use strict';function bq(o){if(!o)return'';const p=new URLSearchParams();if(o.limit)p.set('limit',o.limit);if(o.offset)p.set('offset',o.offset);if(o.prefix)p.set('prefix',o.prefix);if(o.sort_by)p.set('sort_by',o.sort_by);if(o.sort_order)p.set('sort_order',o.sort_order);const s=p.toString();return s?'?'+s:'';}async function pj(r){const ct=r.headers.get('content-type')||'';if(ct.includes('text/html'))throw new Error('Le serveur a renvoye une page HTML au lieu de JSON');return r.json();}async function ag(u,allow404){const r=await fetch(u);if(r.status===404&&allow404)return null;if(!r.ok){let msg='API error: '+r.status;try{const e=await r.clone().json();if(e&&e.detail)msg=e.detail;}catch(_){}throw new Error(msg);}return pj(r);}async function ap(u,d){const r=await fetch(u,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({data:d})});if(!r.ok){let msg='API error: '+r.status;try{const e=await r.clone().json();if(e&&e.detail)msg=e.detail;}catch(_){}throw new Error(msg);}return pj(r);}async function ad(u){const r=await fetch(u,{method:'DELETE'});if(!r.ok){let msg='API error: '+r.status;try{const e=await r.clone().json();if(e&&e.detail)msg=e.detail;}catch(_){}throw new Error(msg);}return pj(r);}function ns(pfx){return{async get(c,k){return ag('/api/app-storage/'+pfx+encodeURIComponent(c)+'/'+encodeURIComponent(k),true);},async set(c,k,d){return ap('/api/app-storage/'+pfx+encodeURIComponent(c)+'/'+encodeURIComponent(k),d);},async delete(c,k){return ad('/api/app-storage/'+pfx+encodeURIComponent(c)+'/'+encodeURIComponent(k));},async list(c,o){return ag('/api/app-storage/'+pfx+encodeURIComponent(c)+bq(o),true);},async getData(k){const s=pfx==='@app/'?'app':'private';return ag('/api/app-data/'+s+'/'+encodeURIComponent(k),true);},async setData(k,v){const s=pfx==='@app/'?'app':'private';return ap('/api/app-data/'+s+'/'+encodeURIComponent(k),v);}};}window.ezgalaxy={storage:ns(''),app:ns('@app/'),isInsideEZGalaxy:true,async ready(){return{status:'ok',mode:'docker'};},configureMobile(){}};window.dispatchEvent(new CustomEvent('ezgalaxy-ready',{detail:window.ezgalaxy}));})();"""
